<template>
  <div class="container" v-if="isLogin">
    ...
    <div class="row justify-content-center px-3">
      <div
        class="col-xl-9 col-md-9 col-12 BG-yellow color_red text-center font18 p-4"
        style="font-weight:400;"
      >
        แจ้งเตือนก่อนฝาก
        <div class="font14 mt-2" style="color:#747474;">
          ลูกค้าทุกท่านต้องมียอดไม่ต่ำกว่า 10 บาท
          <br />มิเช่นนั้น ยอดเทิร์นจะถูกคิดรวมกับของเก่า
        </div>
      </div>
    </div>
    <div class="row justify-content-center p-3">
      <div class="col-xl-9 col-md-9 col-12 BG-pink color_back font18 p-4" style="font-weight:400;">
        ...
        <div class="font14 mt-2" style="color:#606060;">
          ต้องใช้บัญชีที่สมัครฝากเงินเข้ามาเท่านั้น และต้องโอนเงินผ่านช่องทางแอพพลิเคชั่นของธนาคารบนมือถือเท่านั้น มิเช่นนั้นจะไม่สามารถยืนยันรายการฝากได้
          <br />ถ้าเกิดข้อผิดพลาดของระบบให้แจ้งกับพนักงานทันที กรณีที่ไม่แจ้งเราขอสงวนสิทธิ์การถอนเงินทุกกรณี
        </div>
      </div>
    </div>
    <div class="row justify-content-center px-3">
      <div class="col-xl-9 col-md-9 col-12 BG-gray-radius text-center">
        <div v-if="bank[0]">
          ...
          <mdb-btn
            color="warning"
            class="color_back font16 px-3 py-1 mt-2"
            @click="copy_numberbank(bank[0].bank_name, bank[0].number)"
          >คัดลอก</mdb-btn>

          <div class="hr-deposit my-3"></div>
          <div>
            คุณได้โบนัส
            <label class="color_blue">“{{ bonus.title }}”</label>
          </div>
        </div>
        ...
        >**หลังจากโอนเงินแล้ว ระบบจะใช้เวลาดำเนินการไม่เกิน 1 นาที แต่หากเกินเวลาที่กำหนด ให้ติดต่อพนักงานทันที</div>
      </div>
    </div>
    <div class="mt-3"></div>
  </div>
  ...
  mounted() {
    popup_deposit.child("deposit").on("value", (snap) => {
      ...
      for (var i = 0; i < leng.length; i++) {
        if (
          snap.val()[i].status === 1 &&
          momentjs().format("YYYY-MM-DD HH:mm") >= snap.val()[i].date_start &&

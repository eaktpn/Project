  ...
  <div class="container" v-if="isLogin">
    ...
    <div class="row justify-content-center" v-if="codejoin !== 'ไม่มีรหัสผู้แนะนำ'">
      <div class="col-xl-9 col-md-9 col-12 mt-1" style="font-weight:400;">
        รหัสผู้แนะนำคุณ :
        <span style="color:#5E72E4;">{{ codejoin }}</span>
      </div>
    </div>
        ...
        <div class="row justify-content-center">
          ...
          <div class="col-xl-5 col-md-5 col-6 text-right">
            <mdb-btn
              ...
              :class="btn_confirm ? 'btn-gray-aff font14' : 'btn-yellow-aff font14'"
              @click="withdraw()"
            >รับรายได้</mdb-btn>
          </div>
        </div>
        <div class="row justify-content-center">
          <div class="col-11 BG-gray2 text-center color_orange font14 mt-3">
            คุณต้องมียอดเงินมากกว่า 100 บาท
            <br />จึงจะสามารถถอนเงินได้
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="row justify-content-center mt-2" v-if="codesuggestcode !== undefined">
      <div class="col-xl-9 col-md-9 col-12 font16">
        <section class="preview">
          <mdb-input
            basic
            :value="'http://joker24h.abatopup.com/systemaff?join=' + codesuggest"
            disabled
          >
            <mdb-btn
              class="color_white font12"
              color="elegant"
              size="md"
              group
              slot="append"
    ...
    <div class="row justify-content-center mt-2" v-if="codesuggestcode !== undefined">
      <div class="col-xl-10 col-md-10 col-12">
        <social-sharing
          :url="'http://joker24h.abatopup.com/systemaff?join=' + codesuggest"
          title="ลิ้งสมัคร "
          discription="ชวนเพื่อนมาเล่น"
          quote="ชวนเพื่อนมาเล่นสล๊อต"
          hashtags="ชวนเพื่อนมาเล่นสลอต"
          v-cloak
          inline-template
          class="row"
        >
          <div class="row justify-content-center text-center">
            <div class="col-xl-2 col-md-2 col-2 bg-aff-share p-3 m-1">
              <network network="facebook">
                <img
                  src="/images/affiliate/facebook.png"
                  style="width:100%; cursor: pointer; max-width:60px;"
                />
              </network>
            </div>
            <div class="col-xl-2 col-md-2 col-2 bg-aff-share p-3 m-1">
              <a :href="'fb-messenger://share/?link=' + url">
                <img
                  src="/images/affiliate/messenger.png"
                  style="width:100%; cursor: pointer; max-width:60px;"
                />
              </a>
            </div>
            ...
            <div class="col-xl-2 col-md-2 col-2 bg-aff-share p-3 m-1">
              <network network="email">
                <img
                  src="/images/affiliate/gmail.png"
                  style="width:100%; cursor: pointer; max-width:60px;"
                />
              </network>
            </div>
      ...
      <div class="col-xl-9 col-md-9 col-12">
        <div class="row">
          ...
          <div class="col-6 color_white text-right font16">
            <span>
              รวม
              <span class="color_yellow">{{ currencyFormat(logdividend) }}</span> บาท
            </span>
          </div>
        </div>
        <div class="row">
          <div class="col-12 color_white font16">
            ...
            <span>
              <date-picker
                class="input-date"
                valuetype="format"
                style="margin-left:20px; padding-right:5px;"
                lang="en"
                v-model="dateselect"
                @change="selectDate()"
              ></date-picker>
    ...
    <div class="row justify-content-center p-3" style="margin-top:-10px;">
      <div class="col-xl-9 col-md-9 col-12 border-note text-center font14">
        หมายเหตุ ระบบจะทำการคำนวณยอดเงิน
        <br />เวลา 23:00 - 00:00 ของทุกวัน
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    coppyText() {
      this.$copyText(
        "http://joker24h.abatopup.com/systemaff?join=" + this.codesuggest
      );
    ...
    depositWallet() {
      if (parseFloat(this.wallet) > 0) {
        ...
        $(".preloader").show();
        this.$axios
          .post("/walletDeposit", { token: token }, this.token)
          .then((response) => {
            $(".preloader").hide();
            if (response.data.code === "SUCCESS") {
              this.$axios
                .get("/affiliateAddwallet", this.token)
                .then((response) => {
      ...
      if (this.walletSum >= 100) {
        this.$swal({
          ...
          if (result.value) {
            ...
            $(".preloader").show();
            this.$axios
              .post("/walletWithdraw", { token: token }, this.token)
              .then((response) => {
                $(".preloader").hide();
                if (response.data.code === "SUCCESS") {
                  this.$axios.get("/wallet", this.token).then((response) => {
          ...
          for (var i = 0; i < leng.length; i++) {
            if (
              ...
              momentjs().format("YYYY-MM-DD HH:mm") >=
                snap.val()[i].date_start &&
              momentjs().format("YYYY-MM-DD HH:mm") <= snap.val()[i].date_end
            ) {
              show_popup_aff.push({
